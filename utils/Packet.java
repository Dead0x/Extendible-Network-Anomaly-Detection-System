...
public class Packet 
{
	...
	public Field addField(String name, String hex, DTYPE type)
	{
		Field field = new Field(hex, hex.length() * 4);
		field.setFieldName(name);
		field.setDataType(type);
	...
	public Packet peek()
	{
		...
		for (int i = 0; i < frame.frameList.size(); i++)
		{
			if (frame.frameList.get(i).protoType == this.protoType)
			{
				index = (i - 1);
			}
		}
		
		return frame.frameList.get(index);
	}
